...
// Scoring masivo: [{ id, text }] → [{ id, lang, score }] ordenado por score
// descendente. Devuelve SOLO el número (sin detalle de keywords): está pensado
// para barrer cientos de ofertas contra el CV base y filtrar la shortlist;
// el detalle se pide después con matchJob sobre las elegidas. El idioma se
// autodetecta por oferta.
function matchJobsBatch(jobs, base) {
  return jobs
    .map(({ id, text }) => {
      const r = matchJob(text, base);
      return { id, lang: r.lang, score: r.score };
    })
    .sort((a, b) => b.score - a.score);
}
...
module.exports = { STOPWORDS, normalize, tokenize, detectLang, extractKeywords, cvCorpus, matchJob, matchJobsBatch, rankVariants, rankAchievements };
